...
BOOL InitInstance(HANDLE hInstance, int nCmdShow)
{
	...
	GetDBManager()->Init( InsertLogMsg, szDatabase, "sa", "LEGENDOFMIR" );

	// 테이블 읽기
	CConnection *pConn = GetDBManager()->m_dbMain.CreateConnection("LEGENDOFMIR", "sa", "mir3");//( "Mir2_Common", "sa", "prg" );
	if ( pConn )
	{
